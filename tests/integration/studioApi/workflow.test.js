...
const skipTriggerWorkflowTest = !process.env.VV_TEST_WORKFLOW_ID;
...
describeIf(canRunIntegrationTests())('WorkflowManager Integration Tests', () => {
  ...
  describe('triggerWorkflow / terminateWorkflow', () => {
    it.skipIf(skipTriggerWorkflowTest)('should trigger and terminate a workflow', async () => {
      expect(testWorkflowId, 'testWorkflowId should be set by beforeAll').toBeDefined();
  ...
  describe('GetWorkflowHistoryForObject / GetRunningWorkflowForObject', () => {
    ...
    it.skipIf(skipTriggerWorkflowTest)('should retrieve workflow history for an object by its object ID', async () => {
      expect(testWorkflowId, 'testWorkflowId should be set by beforeAll').toBeDefined();
    ...
    it.skipIf(skipTriggerWorkflowTest)('should retrieve running workflow for an object by its object ID', async () => {
      expect(testWorkflowId, 'testWorkflowId should be set by beforeAll').toBeDefined();
      expect(workflowObjectId, 'workflowObjectId should be set by beforeEach').toBeDefined();
      ...
      // expect(workflowRunningData).toHaveProperty('data'); // This endpoint returns 200 with No Content when no running instance is found, so data may not be present
    });
  });
});
