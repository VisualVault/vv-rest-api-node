...
const runEmailDeliveryTest = process.env.VV_TEST_EMAIL_RECIPIENT ? it : it.skip;
...
describeIf(canRunIntegrationTests())('EmailManager Integration Tests', () => {
  ...
  describe('postEmails', () => {
    ...
    runEmailDeliveryTest('should send email to configured recipient', async () => {
  ...
  describe('postEmailsWithAttachments', () => {
    it('should reject email with missing recipients even with attachment', async () => {
      ...
      const filePath = path.join(__dirname, 'fixtures', 'test-form-template.xml');
