...
const runImportTest = process.env.VV_TEST_UNRELEASED_FORM_TEMPLATE_ID_FOR_IMPORT ? it : it.skip;
const runReleaseTest = process.env.VV_TEST_UNRELEASED_FORM_TEMPLATE_ID_FOR_RELEASE ? it : it.skip;
...
describeIf(canRunIntegrationTests())('FormsManager Integration Tests', () => {
  ...
  describe('importFormTemplate', () => {
    ...
    runImportTest('should update an existing form template from XML', async () => {
      const templateId = config.testUnreleasedFormTemplateIdForImport;

      const xmlPath = path.resolve(__dirname, 'fixtures/test-form-template.xml');
  ...
  describe('releaseFormTemplate', () => {
    ...
    runReleaseTest('should release an unreleased form template', async () => {
