...
const runWebServiceTest = process.env.VV_TEST_WEB_SERVICE_NAME ? it : it.skip;
...
describeIf(canRunIntegrationTests())('ScriptsManager Integration Tests', () => {
  ...
  describe('runWebService', () => {
    ...
    runWebServiceTest('should run a web service', async () => {
      ...
      expect(data.meta.status, 'runWebService should return success status').toBe(200);
    });
  });
